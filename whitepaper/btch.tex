...
The authors chose to use a Komodo Asset chain for this new chain for various reasons, but migrating the value to a completely
new chain with a new genesis block is also possible.

It's become common to fork the Bitcoin or Zcash network while inheriting all transaction history, which leads to a cryptocoin that has very little hashpower to protect massive amounts of data. We refer to this as a "worst of all worlds" solution and the ideas in this paper provide working examples of
...
\nsubsection{Bitcoin Spam Attacks}

Somewhat surprisingly, there are \textbf{5568} UTXOs with exactly \textbf{0 BTC} in them, in 816 unique taddrs.
These are overwhelmingly P2PKH UTXOs (99.77\%) and no multisig or native segwit zero satoshi UTXOs were detected.

Much more common are 1 satoshi UTXO, with 871177 UTXOs in 608188 unique taddrs. All UTXO types except segwit native
are present in this set.

These are "unprofitable" UTXOs in the sense that someone would lose more money by claiming their value, due to the
fact that current network fees are greater than those UTXO values. Most likely they are remnants of transaction
spam attacks from miners, who can often earn their money back via fees from the block that the UTXOs occured in.
